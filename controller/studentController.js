...
var doStudentImport = async (req, res) => {
	...
	form.uploadDir = "./uploads/studentExcel"
...
var exportStuduent = (req,res) => {
	var huizongR = []
	var gradeR = ['初一','初二','初三','高一','高二','高三']
	
	function iterator(i) {
		if(i==6) {
			var buffer = xlsx.build(huizongR)
			var fileName = format('学生名单yyyy年MM月dd日', new Date())
			fs.writeFile('./public/'+fileName+'.xlsx',buffer,function(err){
				if (err) {
					console.log(err)
					return
				}
				res.redirect('/'+fileName+'.xlsx')
			})
			return
		}
		Student.find({grade:gradeR[i]}).then((data)=>{
			var sheetR = []
			data.forEach((item)=>{
				sheetR.push([item.sid,item.name,item.grade,item.password])
			})
			huizongR.push({'name':gradeR[i],data:sheetR})
			iterator(++i)
		}).catch((err)=>{})
	}
